...
semantic-memory MCP store (facts + a dependency/structure graph), so Codex can
...
def resolve_binary(explicit):
    ...
    if env and os.access(env, os.X_OK):
        return env
    for cand in (Path.home()/"Coding/Libraries/semantic-memory-mcp/target/release/semantic-memory-mcp",
